...
const ManageDepartments = () => {
  ...
  const handleEditNameTypeData = async (e) => {
    ...
    if (nameType === "department") {
      const data = {
        ...
        category,
        location,
        departmentId: nameTypeData.departmentId,
        employees: nameTypeData.employees,
      };
      ...
      const data = {
        ...
        employeeId: nameTypeData.employeeId,
      };
  ...
  const handleDeleteNameTypeData = async () => {
    console.log(nameType);
    // EDIT BY nameTypeId QUERY
    if (nameType === "department") {
      // DELETE DEPARTMENT QUERY
      try {
        const res = await axios.delete(`/departments/${nameTypeId}/`);
        window.alert(res.data);
        Navigate(`/dashborad/${nameType}`);
      } catch (error) {
        console.log(error);
      }
    } else {
      // DELETE DEPARTMENT QUERY
      try {
        const res = await axios.delete(`/users/${nameTypeId}/`);
        window.alert(res.data);
        Navigate(`/dashborad/${nameType}`);
      } catch (error) {
        console.log(error);
  ...
  const handleAddEmployeeToDepartment = async () => {
    ...
    try {
      if (EmployeeToAdd.length == 0) {
        ...
        await axios.put(`/departments/${nameTypeId}/add`, [
          ...nameTypeData.employees,
        ]);
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      ...
      <div className="managedepartment">
          ...
          <>
                ...
                {nameTypeData?.employees
                  ...
                  .map((row) => (
                    <div className="row" key={row._id}>
                      ...
                      <div className="cell">
                        {row.employeeId ? row.employeeId : "not assigned"}
                      </div>
            ...
            <div className="pagination">
              ...
              <h3
                onClick={() => {
                  page === 3
                    ? console.log("you reach last page")
                    : setPage(page + 1);
                }}
              >
                Next
        ...
        <button onClick={handleDeleteNameTypeData}>
          <i />
          DELETE
        </button>
              ...
              <div className="modalInputs">
                {employees?.map((row) => (
                  <EmployeeListCard
                    key={row._id}
                    employee={row}
                    select={handleSelectEmployeeToAdd}
                  />
                ))}
              </div>
